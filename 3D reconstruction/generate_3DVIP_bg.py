...
flags.DEFINE_string('market_path', '../VIPeR/pytorch/', 'Image to run')
flags.DEFINE_string(
    'json_path', None,
...
def main(dir_path, json_path=None):
    if not os.path.exists('../3D-Person-reID/3DVIP+bg'):
        os.mkdir('../3D-Person-reID/3DVIP+bg')
    sess = tf.Session()
    ...
    #for split in ['gallery', 'query']:
        for root, dirs, files in os.walk(dir_path+split, topdown=True):
            for img_path in files:
                if not ( img_path[-3:]=='jpg' or img_path[-3:]=='png' or img_path[-3:]=='bmp'):
                    continue 
...
def save_mesh(img, img_path, split, proc_param, joints, verts, cam):
    ...
    obj_mesh_name = '../3D-Person-reID/3DVIP+bg/%s/%s/%s.obj'%( split, os.path.basename(os.path.dirname(img_path)), os.path.basename(img_path) )
