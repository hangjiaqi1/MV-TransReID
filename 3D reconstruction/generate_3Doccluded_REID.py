...
def main(dir_path, json_path=None):
    if not os.path.exists('/media/data/data01/wcs/3D-Person-reID/3Doccluded_REID'):
        os.mkdir('/media/data/data01/wcs/3D-Person-reID/3Doccluded_REID')
    sess = tf.Session()
...
def save_mesh(img, img_path, split, proc_param, joints, verts, cam):
    ...
    obj_mesh_name = '/media/data/data01/wcs/3D-Person-reID/3Doccluded_REID/%s/%s/%s.obj'%( split, os.path.basename(os.path.dirname(img_path)), os.path.basename(img_path) )
