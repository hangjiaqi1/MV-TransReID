...
flags.DEFINE_string('market_path', '../3D-Person-reID/Market/pytorch/', 'Image to run')
flags.DEFINE_string(
    'json_path', None,
...
def main(dir_path, json_path=None):
    if not os.path.exists('../3D-Person-reID/3DMarket_Person1'):
        os.mkdir('../3D-Person-reID/3DMarket_Person1')
    sess = tf.Session()
...
def save_mesh(img, img_path, split, proc_param, joints, verts, cam):
    ...
    obj_mesh_name = '../3D-Person-reID/3DMarket_Person1/%s/%s/%s.obj'%( split, os.path.basename(os.path.dirname(img_path)), os.path.basename(img_path) )
