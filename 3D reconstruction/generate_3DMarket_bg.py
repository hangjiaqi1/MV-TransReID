...
flags.DEFINE_string('market_path', '/media/data/data01/wcs/3D-Person-reID/market1501', 'Image to run')
flags.DEFINE_string(
    'json_path', None,
...
def main(dir_path, json_path=None):
    if not os.path.exists('/media/data/data01/wcs/3D-Person-reID/3Dmarket1501+bg'):
        os.mkdir('/media/data/data01/wcs/3D-Person-reID/3Dmarket1501+bg')
    sess = tf.Session()
...
def save_mesh(img, img_path, split, proc_param, joints, verts, cam, faces):
    ...
    img_copy = img.copy()
    obj_mesh_name = '/media/data/data01/wcs/3D-Person-reID/3Dmarket1501+bg/%s/%s/%s.obj'%( split, os.path.basename(os.path.dirname(img_path)), os.path.basename(img_path) )
