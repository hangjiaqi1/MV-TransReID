...
flags.DEFINE_string('market_path', '/media/data/data01/wcs/3D-Person-reID/Occluded_REID/', 'Image to run')
flags.DEFINE_string(
    'json_path', None,
...
def main(dir_path, json_path=None):
    if not os.path.exists('/media/data/data01/wcs/3D-Person-reID/3D-Occluded_REID+bg'):
        os.mkdir('/media/data/data01/wcs/3D-Person-reID/3D-Occluded_REID+bg')
    sess = tf.Session()
    model = RunModel(config, sess=sess)
    for split in ['bounding_box_test', 'query']:
...
def save_mesh(img, img_path, split, proc_param, joints, verts, cam):
    ...
    obj_mesh_name = '/media/data/data01/wcs/3D-Person-reID/3D-Occluded_REID+bg/%s/%s.obj'%( split, os.path.basename(img_path))
