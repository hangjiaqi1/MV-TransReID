...
def main(dir_path, json_path=None):
    if not os.path.exists('../3D-Person-reID/3DMarket_Person'):
        os.mkdir('../3D-Person-reID/3DMarket_Person')
    sess = tf.Session()
...
def save_mesh(img, img_path, split, proc_param, joints, verts, cam):
    ...
    obj_mesh_name = '../3D-Person-reID/3DMarket_Person/%s/%s/%s.obj'%( split, os.path.basename(os.path.dirname(img_path)), os.path.basename(img_path) )
