...
flags.DEFINE_string('market_path', '../3D-Person-reID/prcc/rgb/', 'Image to run')
flags.DEFINE_string(
    'json_path', None,
...
def main(dir_path, json_path=None):
    if not os.path.exists('../3D-Person-reID/3DPrcc+bg'):
        os.mkdir('../3D-Person-reID/3DPrcc+bg')
    sess = tf.Session()
    model = RunModel(config, sess=sess)
    face_path = './src/tf_smpl/smpl_faces.npy'
    faces = np.load(face_path)
    count = 0 
    for split in ['gallary']:
        if split =='test':
            if not os.path.exists('../3D-Person-reID/3DPrcc+bg/test'):
                os.mkdir('../3D-Person-reID/3DPrcc+bg/test')
            for split1 in ['A','B','C']:
                for root, dirs, files in os.walk(dir_path+split+split1, topdown=True):
                    for img_path in files:
                        ...
                        save_mesh(img, img_path, split1, proc_param, joints[0], verts[0], cams[0], faces)
                        count +=1
                        #if count == 100:
                        #    break
        else:
...
def save_mesh(img, img_path, split, proc_param, joints, verts, cam, faces):
    ...
    img_copy = img.copy()
    obj_mesh_name = '../3D-Person-reID/3DPrcc+bg/%s/%s/%s.obj'%( split, os.path.basename(os.path.dirname(img_path)), os.path.basename(img_path) )
