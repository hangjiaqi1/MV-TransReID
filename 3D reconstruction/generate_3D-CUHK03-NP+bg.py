...
flags.DEFINE_string('market_path', '/media/data/data01/wcs/3D-Person-reID/CUHK03-NP/', 'Image to run')
flags.DEFINE_string(
    'json_path', None,
...
def main(dir_path, json_path=None):
    if not os.path.exists('/media/data/data01/wcs/3D-Person-reID//3D-CUHK03-NP+bg'):
        os.mkdir('/media/data/data01/wcs/3D-Person-reID//3D-CUHK03-NP+bg')
    sess = tf.Session()
...
def save_mesh(img, img_path, split, proc_param, joints, verts, cam):
    ...
    obj_mesh_name = '/media/data/data01/wcs/3D-Person-reID//3D-CUHK03-NP+bg/%s/%s.obj'%( split, os.path.basename(img_path) )
